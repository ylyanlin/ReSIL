...
def approximate_type(type_str):
	...
	if type_str[-1] == '*'or type_str == 'pointer' or type_str == 'func_ptr' or type_str.split()[0][-1] == '*'  or type_str.find('* [') != -1 or type_str.find('&') != -1:
		return 'pointer'
	...
	for whole_func_name in func_list:
		...
		if file_name in extracted_pickle:
			...
			if temp_data.shape[0] < max_length:
				...
				extract_info[func_tag]['length'] = temp_data.shape[0]
			extract_info[func_tag]['data'] = temp_data
			temp_type = approximate_type(file_info[func_name]['args_type'][arg_no])
...
class Dataset(object):
	def __init__(self, data_folder, func_path, embed_path, thread_num, embed_dim, max_length, class_num, tag):
		global embed_info,package2whole,extract_vector,extract_vector_test
		...
		if self.tag == 'num_args':
			extract_vector = get_vector(self.data_folder, self.train_func_list, self.embed_dim, self.max_length, self.class_num,
									embed_info)
			#extract_vector_test = get_vector(self.data_folder, self.test_func_list, self.embed_dim, self.max_length, self.class_num,
                                                                                                        #embed_info)
		else:
	...
	def get_batch_cv(self, batch_size):
		...
		if start + batch_size >= len(self.train_func_list):
			self.test_tag = False
			func_list_batch = self.train_func_list[start:]
	...
	def get_batch_data_cv(self, batch_func_list):
		...
		for whole_func_name in func_list:
			if whole_func_name in extract_vector:
				print("in",whole_func_name,extract_vector[whole_func_name])
				batch_info[whole_func_name] = extract_vector[whole_func_name]
