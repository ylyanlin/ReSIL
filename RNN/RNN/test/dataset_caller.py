...
def get_vector(folder_path, func_list, embed_dim, max_length, class_num, embed_info):
	...
	for whole_func_name in func_list:
		'''callee_name#caller_name#indice'''
		'''ours filename#callee_name#indice#addr'''
		temp = whole_func_name.split('#')
		file_name =  temp[0]
		callee_name = temp[1]
		indice = int(temp[2])
		...
		temp_data = []
		indice_list = sorted(range(indice), reverse=True)
		if callee_name in file_info['functions']:
			print(whole_func_name)
			if indice_list[0] < len(file_info['functions'][callee_name]['inst_bytes']):
				for indice_id in indice_list:
					#print(indice_id, len(file_info['functions'][callee_name]['inst_bytes']))
					inst = file_info['functions'][callee_name]['inst_bytes'][indice_id-1]
					if str(inst) in embed_info:
						temp_data.append(embed_info[str(inst)]['vector'])
					else:
						temp_data.append([0.0] * embed_dim)
				...
				extract_info[func_tag]['label'] = one_hot_encoding(file_info['icall'][( callee_name, indice,addr)]['num_args'],
																   class_num)
...
def get_single_num_args(folder_path, file_name, func_list, embed_dim, max_length, class_num):
	...
	with open(file_path,'rb') as f:
		file_info = pickle.load(f,encoding='latin1')
		print(file_info['functions'])
	for whole_func_name in func_list:
		whole_func_name = whole_func_name.rstrip()
		'''filename_name#callee_name#indice#addr'''
		temp = whole_func_name.split('#')

		print(whole_func_name,temp)
		file_name =  temp[0]
		print("file_name",file_name)
		callee_name = temp[1]
		print("callee_name",callee_name)
		indice_id = int(temp[2])
		print("indice_id",indice_id)
		...
		if callee_name in file_info['functions']:
			print(whole_func_name)
			if indice_id < len(file_info['functions'][callee_name]['inst_bytes']):
				
				print(indice_id, len(file_info['functions'][callee_name]['inst_bytes']))
				inst = file_info['functions'][callee_name]['inst_bytes'][indice_id]
				if str(inst) in embed_info:
					temp_data.append(embed_info[str(inst)]['vector'])
				else:
					temp_data.append([0.0] * embed_dim)
				...
				extract_info[func_tag]['label'] = one_hot_encoding(file_info['icall'][( callee_name, indice)]['num_args'],
																   class_num)
...
class Dataset(object):
	def __init__(self, data_folder, func_path, embed_path, thread_num, embed_dim, max_length, class_num, tag):
		...
		if self.tag == 'num_args':
			extract_vector = get_vector(data_folder, self.func_list, self.embed_dim, self.max_length, self.class_num,
									embed_info)
	...
	def get_batch_data(self, batch_func_list):
		...
		for whole_func_name in func_list:
			if whole_func_name in extract_vector:
				if len(extract_vector[whole_func_name]) != 0:
		...
		if self.tag == 'num_args':
			for whole_func_name in func_list:
				temp = whole_func_name
				if binary_name == '':
					binary_name = whole_func_name.split('#')[0]
					input_func_list.append(temp)
					...
					if binary_name == whole_func_name.split('#')[0]:
						...
						binary_name = whole_func_name.split('#')[0]
						input_func_list = ['#'.join(whole_func_name.split('#')[0:])]
