...
static int process_function_lib(ArmsFunction *f, void *arg) {
    ArmsLiveness *alive = (ArmsLiveness *) arg;
    alive->get_arg_count_lib(f);
...
static int print_regulars(ArmsFunction *f, void *arg) {
    ...
    #ifdef FIRST_INST
    std::vector<std::vector<unsigned long>>readoffsets;
    readoffsets = f->get_offset();
    for (int i = 0; i < IA64_ARGS; i++)
    {
        fprintf(fp,"reading for register %d: ",i);
...
void getParams_lib(BPatch_image *image, BPatch_addressSpace *as, CFG *cfg) {
    ...
    if (cfg->foreach_function(process_function_lib, &alive) < 0) {
        fcfiPassLog("Could not analyze functions");
        return;
    }
  ...
  class FCFIPass : public ModulePass {
    ...
    public:
        ...
        virtual bool runOnModule(void *M) {
            ...
            std::map<string,string> obj2path;
            ...
            for (unsigned i = 0; i < objs.size(); i++)
            {
                ...
                //obj2path[objname] = pathname;

                if (objname.find(".so") != string::npos)
                {
                    printf("* library %s\n", objname.c_str());
                    BPatch_addressSpace* handle = NULL;
                    handle = bpatch.openBinary(pathname.c_str(), false);
                    if (!handle) { 
                        printf("openBinary failed\n"); 
                        continue;
                    }
                    cfg = dyninst_build_cfg_lib(handle, pathname.c_str());
                    
                    char output[512];
                    char suffix[4]="txt";
                    sprintf(output,"%s.%s.%s",optBinfo.getValue().c_str(),objname.c_str(),suffix);
                    fp = fopen(output,"w");
                    fprintf(fp,"Object: %s\n", objname.c_str());
                    ...
                    if(!cfg) {
                    printf("CFG generation failed");
                    return false;
                    }
                    
                    fprintf(fp,"Functions: %lu\n", cfg->count_functions());
                    
                    getParams_lib(image, as, cfg);
                    fprintf(fp,"\n[varargs]\n");
            ...
            /*for (unsigned i = 0; i <mods->size(); i++)
            {
                ...
                if (objname.find(".so") != string::npos)
                {
                    ...
                    printf("\tend addr is %lx\n",lib->getBaseAddr() + lib->getSize());

                    if (obj2path.find(objname) != obj2path.end())
                    {

                        BPatch_addressSpace* handle = NULL;
                        handle = bpatch.openBinary(obj2path[objname].c_str(), false);
                        if (!handle) { 
                            printf("openBinary failed\n"); 
                            continue;
                        }
                        
                        cfg = dyninst_build_cfg(handle, i);
                        char output[512];
