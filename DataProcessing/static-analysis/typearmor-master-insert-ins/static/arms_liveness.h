...
public: 
    ...
    //by yanlin
    #ifdef FIRST_INST
    std::vector<unsigned long> getReadOffsets(ArmsBasicBlock *block);
...
#define IA64_RSP 14 /* to identify varargs */
#define IA64_RBP 15
#define IA64_RAX 16

#define IA64_LAST_ARG IA64_R9
#define IA64_REGS 17 /* Number of registers that we keep track of */
#define IA64_ARGS 14 /* Number of registers used for arguments. Additional
                       registers (like RSP) must be placed after the register
                       arguments. */

#define IA64_INT_ARGS 6
...
public:
    ...
    int get_arg_count_lib(ArmsFunction *f);
...
private:
    ...
    void parse_register(Instruction::Ptr iptr, RegisterAST::Ptr reg, 
                        ArmsBasicBlock *bb, unsigned long offset, StateType state);
    void parse_register_set(Instruction::Ptr iptr, std::set<RegisterAST::Ptr> register_set, 
                            ArmsBasicBlock *bb, unsigned long offset, StateType state);
    ...
    std::pair<uint32_t, std::vector<std::vector<unsigned long>>> getForwardLiveness_lib(ArmsFunction *f,
                                     ArmsBasicBlock *bb,
    ...
    uint16_t getForwardLiveness2(ArmsFunction *f,
            ...
            std::vector<ArmsBasicBlock *> argcount_analyzed_blocks);
    #else 
    std::pair<uint32_t, std::vector<std::vector<unsigned long>>> getForwardLiveness2(ArmsFunction *f,
                                     ArmsBasicBlock *bb,
            std::vector<ArmsBasicBlock *> fts,
            std::vector<ArmsBasicBlock *> argcount_analyzed_blocks);
    #endif
    ...
    uint32_t getBackwardLiveness(ArmsFunction *f,
                                       ArmsBasicBlock *bb,
    ...
    std::map <ArmsBasicBlock *,std::pair<uint16_t, std::vector<std::vector<unsigned long>>>> forward_cache;
