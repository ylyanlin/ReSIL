...
//by yanlin, obtain the first instructions that read the argument registers
#define FIRST_INST
...
//  dprintf("setState(%p, %p, %d) (this: %p)\n", block, (void *) offset, state, this);
    ...
    if (block_list[block][offset] == IA64_READ  && state == IA64_WRITE)
    {
        block_list[block][offset] = IA64_READ;
    }
     else {
...
//by yanlin
#ifdef FIRST_INST
std::vector<unsigned long> ArmsRegister::getReadOffsets(ArmsBasicBlock *block)
{
    ...
    for (state_it_type it  = block_list[block].begin(); 
        ...
        StateType reg_state = it->second;

        /*while (reg_state != IA64_WRITE)
        {
            offsets.push_back(insn_offset);
        }*/
        if (reg_state == IA64_WRITE)
            break;
        else if (reg_state == IA64_READ)
        {
            offsets.push_back(insn_offset);
        }
    }
...
string ArmsLiveness::get_register_name(int reg) {
    switch(reg) {
        ...
        #ifdef FIRST_INST
        case IA64_XMM0: return "XMM0";
        case IA64_XMM1: return "XMM1";
        case IA64_XMM2: return "XMM2";
        case IA64_XMM3: return "XMM3";
        case IA64_XMM4: return "XMM4";
        case IA64_XMM5: return "XMM5";
        case IA64_XMM6: return "XMM6";
        case IA64_XMM7: return "XMM7";
        #endif
...
int get_reg_index(RegisterAST::Ptr reg) {
    ...
    if (reg_class != x86::GPR && reg_class !=x86::XMM) {
...
void ArmsLiveness::parse_register(Instruction::Ptr iptr,RegisterAST::Ptr reg, 
    ...
    if (found == std::string::npos)
        ...
        rw_registers[reg_index].setState(bb, offset, IA64_WRITE);
    //rw_registers[reg_index].setState(bb, offset, state);
    
}

void ArmsLiveness::parse_register_set(Instruction::Ptr iptr,std::set<RegisterAST::Ptr> register_set, 
                        ArmsBasicBlock *bb, unsigned long offset, StateType state) {
    ...
    for (std::set<RegisterAST::Ptr>::iterator it  = register_set.begin(); 
        ...
        int  reg_value = reg->getID().val();
        parse_register(iptr, reg, bb, offset, state);
    }
}
...
void ArmsLiveness::parse_instruction(Instruction::Ptr iptr,
    ...
    if (iptr->size() == 2 && iptr->rawByte(0) == 0x0F && iptr->rawByte(1) == 0xA2) {
        ...
        parse_register(iptr, eax, bb, offset, IA64_WRITE);
        parse_register(iptr, ebx, bb, offset, IA64_WRITE);
        parse_register(iptr, ecx, bb, offset, IA64_WRITE);
        parse_register(iptr, edx, bb, offset, IA64_WRITE);
        return;
    }
 ...
 */
    ...
    parse_register_set(iptr, register_set, bb, offset, IA64_READ);
    register_set.clear();

    if (!isNop(iptr)) iptr->getWriteSet(register_set);
    parse_register_set(iptr, register_set, bb, offset, IA64_WRITE);
    register_set.clear();
...
int ArmsLiveness::is_variadic(ArmsFunction *f) {
    ...
    int n = 0;
    dprintf("  [varargs.C1] Searching for last n arguments that are read before write... %s\n",f->get_name().c_str());
    for (i = IA64_LAST_ARG; i >= n; i--) {
        dprintf("  [varargs.C1] %d: %s - %s\n", i, get_register_name(i   ).c_str(), 
    ...
    dprintf("  [varargs.C3] Comparing offsets...\n");
    ...
    dprintf("  [varargs.C4] Testing whether RSP/RBP was read...\n");
...
void set_reg_bitmap(uint32_t *reg_bitmap, int reg_index, uint32_t state) {
    *reg_bitmap &= ~(0x03 << (reg_index * 2)); // clear first
    *reg_bitmap |= state << (reg_index * 2);
}

/* returns true if register <reg_index> is set to <state> */
bool is_reg_bitmap(int reg_bitmap, int reg_index, uint32_t state) {
...
int bitmap_argset(int reg_bitmap, uint32_t state) {
    int argcount = 0;
...
uint32_t ArmsLiveness::getBackwardLiveness(ArmsFunction *f,
                                       ArmsBasicBlock *bb,
    ...
    bitmaps = (uint32_t *) malloc(bb->incoming_edge_count() * sizeof(uint32_t)); 
...
//      ddprintf("[bt]%s best_bitmap = %x | reg_bitmap = %x\n", blanks.c_str(), best_bitmap, reg_bitmap);
        ...
        uint32_t best_bitmap = 0xaaaaaaa;
...
std::pair<uint32_t, std::vector<std::vector<unsigned long>>> ArmsLiveness::getForwardLiveness_lib(ArmsFunction *f,
                                       ArmsBasicBlock *bb,
                                       ...
                                       std::vector<ArmsBasicBlock *> argcount_analyzed_blocks)
{
    ...
    //by yanlin
    std::vector<std::vector<unsigned long>>readoffsets (IA64_ARGS,std::vector<unsigned long>());

    std::vector<std::vector<std::vector<unsigned long>>> temp_offsets;
    uint32_t reg_bitmap = 0;
    for (int i = 0; i < IA64_ARGS; i++) {
    ...
    for (int i = 0; i < IA64_ARGS; i++) {
        if (is_reg_bitmap(reg_bitmap, i, IA64_CLEAR)) {
            ...
            StateType state;
            std::vector<unsigned long> insn_offsets;
            if (bb->is_vararg_mark()) {
                ...
                insn_offsets = rw_registers[i].getReadOffsets(bb);
                

                if (state == IA64_READ) {
                    for (std::vector<unsigned long>::iterator it = insn_offsets.begin() ; it != insn_offsets.end(); ++it)
                    {
                        dprintf("possible reading %lx for register %d\n",*it,i);
                        readoffsets[i].push_back(*it);
                    }
    ...
    for (size_t i = 0; i < bb->outgoing_edge_count(); i++) {
        ...
        // by yanlin
        temp_offsets.push_back(readoffsets);
...
//          assert(!isNop(fallthrough_bb));
    ...
    if (edges_followed == 0) {
        ...
        uint32_t best_bitmap = 0xfffffff; /* start with worst-case, all clear */
        // by yanlin
        std::vector<std::vector<unsigned long>> possible_readoffsets (IA64_ARGS,std::vector<unsigned long>()) ;
        for(int i=0; i < IA64_ARGS;i++)
        {
            for (std::vector<unsigned long>::iterator it = readoffsets[i].begin() ; it != readoffsets[i].end(); ++it)
            {
                        possible_readoffsets[i].push_back(*it);  
            }
        }
        ...
        for ( int t = 0 ; t < edges_followed; t++){
            uint32_t bitmap = bitmaps[t];
            std::vector<std::vector<unsigned long>>block_read = temp_offsets[t];
            ...
            for (int i = 0; i < IA64_ARGS; i++) {
                ...
                if (is_reg_bitmap(best_bitmap, i, 0x3)) {
                    set_reg_bitmap(&best_bitmap, i, get_reg_bitmap(bitmap, i));
                    for (std::vector<unsigned long>::iterator it = block_read[i].begin() ; it != block_read[i].end(); ++it)
                    {
                        possible_readoffsets[i].push_back(*it);  
                    }
                    ...
                    if (is_reg_bitmap(bitmap, i, IA64_READ)) {
                        ...
                        //possible_readoffsets.push_back(block_read);
        ...
        for (int i = 0; i < IA64_ARGS; i++) {
            if (is_reg_bitmap(reg_bitmap, i, IA64_CLEAR)) {
                set_reg_bitmap(&reg_bitmap, i, get_reg_bitmap(best_bitmap,i));
                // by yanlin
                for (std::vector<unsigned long>::iterator it = possible_readoffsets[i].begin() ; it != possible_readoffsets[i].end(); ++it)
                {
                    readoffsets[i].push_back(*it);
                }
            }
...
ft_debug_return:
    ...
    /*for (int i = 0; i < IA64_ARGS; i++)
    {
        dprintf("reading for register %d:",i);
        for (std::vector<unsigned long>::iterator it = readoffsets[i].begin() ; it != readoffsets[i].end(); ++it)
        {
            dprintf("%lx ",*it);
        }
    ...
    std::pair<uint32_t, std::vector<std::vector<unsigned long>>> result;
    result.first = reg_bitmap;
    result.second=readoffsets;
    return result;
}
...
#ifdef FIRST_INST
std::pair<uint32_t, std::vector<std::vector<unsigned long>>> ArmsLiveness::getForwardLiveness2(ArmsFunction *f,
                                       ArmsBasicBlock *bb,
                                       std::vector<ArmsBasicBlock *> fts,
                                       std::vector<ArmsBasicBlock *> argcount_analyzed_blocks)
{
    ...
    if (edges_followed == 0) {
        ...
        for (int i = 0; i < IA64_ARGS; i++) {
            if (is_reg_bitmap(reg_bitmap, i, IA64_CLEAR)) {
                ...
                for (std::vector<unsigned long>::iterator it = possible_readoffsets[i].begin() ; it != possible_readoffsets[i].end(); ++it)
                {
                   dprintf("combine possible reading %lx for register %d\n",*it,i);
                    readoffsets[i].push_back(*it);
                }
            }
        }
...
int ArmsLiveness::get_arg_count_lib(ArmsFunction *f)
{
    /* Only if this function was analyzed */
    if (function_blocks.count(f) == 0) {
        return -1;
    ...
    std::pair<uint32_t, std::vector<std::vector<unsigned long>>> result;
    std::vector<std::vector<unsigned long>>offsets;
    result = getForwardLiveness_lib(f, entry_blocks[0], fts, argcount_analyzed_blocks);
    reg_bitmap = result.first;
    offsets = result.second;
    int argcount = 0;
...
int ArmsLiveness::get_arg_count(ArmsFunction *f) {
    ...
    std::pair<uint32_t, std::vector<std::vector<unsigned long>>> result;
    std::vector<std::vector<unsigned long>>offsets;
    result = getForwardLiveness2(f, entry_blocks[0], fts, argcount_analyzed_blocks);
    reg_bitmap = result.first;
    offsets = result.second;
    int argcount = 0;
    for (int i = IA64_ARGS; i >=0; i--) {
