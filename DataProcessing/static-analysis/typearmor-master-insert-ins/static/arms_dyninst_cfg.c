...
//by yanlin
void 
DICFG::insert_functions_and_bbs_lib(std::vector<BPatch_function *> *funcs)
{
	...
	{
		fun = Dyninst::ParseAPI::convert(*funcs_iter);
...
DICFG::insert_functions_and_bbs(const CodeObject::funclist& funcs)
{
	...
	for(funcs_iter = funcs.begin(); funcs_iter != funcs.end(); funcs_iter++) {
		fun = *funcs_iter;
		//printf("function name and addr %s-%lx\n",fun->name().c_str(),(address_t)fun->addr());
		if (find_function((address_t)fun->addr())) continue; 
		arms_fun = new ArmsFunction((address_t)fun->addr(), fun->name(), this);
...
// by yanlin
void 
DICFG::insert_edges_lib(std::vector<BPatch_function *> *funcs)
{
...
std::vector<relocationEntry> getPLT(BPatch_module *module) {
...
DICFG* dyninst_build_cfg_lib(BPatch_addressSpace *handle, const char *filename)
{
	SymtabCodeSource *sts;
	CodeObject *co;


	sts 	= new SymtabCodeSource((char*)filename);
	co = new CodeObject( sts );


	DICFG *parse_cfg = new DICFG(filename); 
	...
	//funcs = image->getProcedures(false);
	for (mods_iter = mods->begin(); mods_iter != mods->end(); mods_iter++) {
		//std::vector<relocationEntry> plt = getPLT(*mods_iter);
		//printf("* Found %lu PLT entries that we have to analyze:\n", plt.size());
		...
		funcs = (*mods_iter)->getProcedures(false); 
		std::vector<BPatch_function *>::iterator funcs_iter = funcs->begin();
		
		
		for(; funcs_iter != funcs->end(); funcs_iter++) {
			//printf("function name %s->%lx\n",(*funcs_iter)->getName().c_str(),(*funcs_iter)->getBaseAddr());
			co->parse((Address)(*funcs_iter)->getBaseAddr(), true);
		} 

		//parse_cfg->insert_functions_and_bbs_lib(funcs);	
		//parse_cfg->insert_edges_lib(funcs);	
	}

	for(auto funcs_iter=funcs->begin(); funcs_iter != funcs->end(); funcs_iter++) {
			unsigned long baseAddr,lastAddr;
			(*funcs_iter)->getAddressRange(baseAddr, lastAddr);
	...
	/* Parse PLT entries */
	Symtab *symtab	= Symtab::findOpenSymtab(string(filename));
	vector<SymtabAPI::relocationEntry> fbt;
	vector<SymtabAPI::relocationEntry>::iterator fbt_iter;
	...
	const CodeObject::funclist& funcs_obj = co->funcs();
	
	parse_cfg->insert_functions_and_bbs(funcs_obj);		
	for (fbt_iter = fbt.begin(); fbt_iter != fbt.end(); fbt_iter++) {
	...
	parse_cfg->insert_edges(funcs_obj);
